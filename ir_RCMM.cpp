...
bool IRrecv::decodeRCMM(decode_results *results)
{
    ...
    if (!WITHIN(val, RCMM_HDR_SPACE, RCMM_HDR_TOLERANCE)) return false ;

    while (offset < (results->rawlen - 1)) {
        data <<= 2 ;
        val = (results->rawbuf[offset++] + results->rawbuf[offset++]) - RCMM_DATA_MARK_TICKS ;
        if      (WITHIN(val, RCMM_ZERO_SPACE, RCMM_DATA_SPACE_TOLERANCE)) ;
        else if (WITHIN(val, RCMM_ONE_SPACE, RCMM_DATA_SPACE_TOLERANCE))   data += 1 ;
        else if (WITHIN(val, RCMM_TWO_SPACE, RCMM_DATA_SPACE_TOLERANCE))   data += 2 ;
        else if (WITHIN(val, RCMM_THREE_SPACE, RCMM_DATA_SPACE_TOLERANCE)) data += 3 ;
        else                                                               return false ;
    }
